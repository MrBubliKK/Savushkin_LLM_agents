...
from sc_client.client import template_search, search_links_by_contents, get_link_content
...
class CallAgent(ScAgentClassic):
    ...
    def run(self, action_node: ScAddr) -> ScResult:
        ...
        subject_area_prompt = f"""
          ...
          5. "Схемы и Процессы": Эта область включает запросы, касающиеся схем, алгоритмов, процессов и последовательностей действий. Агенты этой области отвечают на вопросы об этапах схем, последовательности шагов в процессах, описании алгоритмов и общей структуре схем. Запросы в этой области направлены на понимание того, **как что-то работает**, **какие шаги включает процесс**, и **какова последовательность действий** в схеме или алгоритме.
              Примеры запросов:
                - "Покажи этапы схемы производственного процесса"
                - "Какая последовательность действий в алгоритме шифрования?"
                - "Расскажи схему работы двигателя внутреннего сгорания"
                - "Этапы процесса приготовления борща"
                - "Последовательность шагов в инструкции по сборке мебели"
                - "Как работает схема электронной подписи?"

          6. "Общие Запросы": Эта область предназначена для обработки запросов, которые не относятся к поиску структуры, описаний, классификации, семантических связей или схем сущностей. Сюда попадают запросы общего характера, не связанные напрямую с характеристиками, иерархией, классификацией, знаниями или схемами сущностей.
              Примеры запросов:
                - "Сколько будет 2 + 2?"
        ...
        if subject_area_prompt_answer == "Структура и Иерархия":
            ...
            return ScResult.OK
        elif subject_area_prompt_answer == "Схемы и Процессы":
            self.schemes_and_processes(action_node, link_query, api_key)
            return ScResult.OK
    ...
    def schemes_and_processes(self, action_node, link_query, api_key):
        area_description_prompt = f"""
          Запрос пользователя: {link_query}

          Ты - система обработки запросов, которая классифицирует входящие запросы и извлекает из них название схемы, если это необходимо.  Твоя задача - обрабатывать запросы, относящиеся к предметной области "Схемы и Процессы", **и ТОЛЬКО к запросам о шагах и этапах известных схем из следующего списка:**

          **Список известных схем:**
          - "Схема аппаратной процедуры для приготовления творожной массы"
          - "Схема технологического процесса производства кефира"
          - "Схема автоматизированной линии розлива молока"
          - "Схема контроля качества молочной продукции"
          ...
          Предметная область "Схемы и Процессы":

          Эта область **ОГРАНИЧЕНА** запросами, касающимися схем, алгоритмов, процессов и последовательностей действий, **ТОЛЬКО в контексте известных схем из списка выше**.  Агенты этой области отвечают **ИСКЛЮЧИТЕЛЬНО** на вопросы об этапах известных схем, последовательности шагов в процессах, описании алгоритмов и общей структуре **известных схем**. Запросы в этой области направлены **ТОЛЬКО** на понимание того, **как работает ИЗВЕСТНАЯ схема**, **какие шаги включает ИЗВЕСТНЫЙ процесс**, и **какова последовательность действий в ИЗВЕСТНОЙ схеме или алгоритме**.

          Типы запросов для предметной области "Схемы и Процессы":

          1. "scheme_steps_needed": запрос на описание **этапов схемы, последовательности действий или шагов процесса ДЛЯ ОДНОЙ ИЗ ИЗВЕСТНЫХ СХЕМ**.  Этот тип запроса **ЕДИНСТВЕННЫЙ** и **ОСНОВНОЙ** для данной предметной области. Он ориентирован на получение информации о том, как устроена **известная** схема или процесс, какие этапы или шаги она включает, и в какой последовательности они выполняются.  Запрос **ДОЛЖЕН** явно упоминать название схемы или подразумевать ее из контекста **В ОТНОШЕНИИ К ИЗВЕСТНЫМ СХЕМАМ**.
              Примеры запросов: "покажи этапы схемы приготовления творожной массы", "какая последовательность действий в схеме производства кефира?", "расскажи схему линии розлива молока", "этапы схемы контроля качества продукции".
              Ключевые слова и фразы-индикаторы: "этапы схемы", "последовательность действий", "последовательность шагов", "схема работы", "как работает", "опиши процесс", "алгоритм", "шаги процесса", "стадии схемы".  **КЛЮЧЕВОЕ УСЛОВИЕ: в запросе ДОЛЖЕН БЫТЬ контекст, указывающий на ОДНУ ИЗ ИЗВЕСТНЫХ СХЕМ.**

          Извлечение сущности - КЛЮЧЕВОЕ ПРАВИЛО для предметной области "Схемы и Процессы":

          Если тип запроса "scheme_steps_needed":

          **Поиск названия схемы в запросе:**

          1. **Проверь, содержит ли запрос пользователя ТОЧНОЕ НАЗВАНИЕ ОДНОЙ ИЗ ИЗВЕСТНЫХ СХЕМ из списка выше.**  Поиск должен быть **точным соответствием** (регистр важен, если необходимо, или можно сделать нечувствительным к регистру, если нужно).

          2. **Если ТОЧНОЕ НАЗВАНИЕ СХЕМЫ НАЙДЕНО в запросе (независимо от кавычек):**
             - **Используй НАЙДЕННОЕ ТОЧНОЕ НАЗВАНИЕ СХЕМЫ как значение для "entity_name".**

          3. **Если ТОЧНОЕ НАЗВАНИЕ СХЕМЫ НЕ НАЙДЕНО в запросе:**
             - **В этом случае, поскольку область ограничена известными схемами, поле "entity_name" должно оставаться ПУСТЫМ.**  **Не применять правило кавычек и не пытаться извлекать другие названия схем или процессов, если точное название из списка не найдено.**  **Если запрос не относится к этапам ИЗВЕСТНОЙ схемы, он НЕ ПОДЛЕЖИТ обработке в данной предметной области.**

          Формат ответа:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "название схемы из списка (если найдено) / '' (пусто, если точное название не найдено)"
          }}

          **Примеры:**

          Пример 1:
          Запрос: "Покажи этапы схемы аппаратной процедуры для приготовления творожной массы"
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема аппаратной процедуры для приготовления творожной массы"
          }}

          Пример 2:
          Запрос: "Какая последовательность действий в схеме технологического процесса производства кефира?"
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема технологического процесса производства кефира"
          }}

          Пример 3:
          Запрос: "Расскажи схему автоматизированной линии розлива молока"
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема автоматизированной линии розлива молока"
          }}

          Пример 4:
          Запрос: "Этапы схемы контроля качества молочной продукции"
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема контроля качества молочной продукции"
          }}

          Пример 5:
          Запрос: "Этапы 'Схема аппаратной процедуры для приготовления творожной массы'?"  **<-- В кавычках, но точное название**
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема аппаратной процедуры для приготовления творожной массы"
          ...
          Пример 6:
          Запрос: "Хочу узнать про этапы схемы для приготовления творога" **<-- Упоминание "творога" - намек на известную схему**
          Ответ:
          {{
            "decision": "scheme_steps_needed",
            "entity_name": "Схема аппаратной процедуры для приготовления творожной массы"
          }}  **(Внимание: этот пример требует более сложной логики, чем просто точное соответствие.  Модель должна уметь сопоставлять "приготовления творога" с "приготовления творожной массы".  В текущем промпте это делается за счет примера. В реальности может потребоваться более продвинутый механизм сопоставления или ключевых слов)**

          Пример 7:
          Запрос: "Расскажи общую схему производственного процесса на молочном заводе" **<-- Общий запрос, не про конкретную ИЗВЕСТНУЮ схему из списка**
          Ответ:
          {{
            "decision": "general_query", # или, возможно, лучше классифицировать как что-то другое, если есть внешний классификатор
            "entity_name": ""
          }}

          Пример 8:
          Запрос: "Какие есть схемы на вашем предприятии?" **<-- Общий вопрос о схемах, не про этапы конкретной ИЗВЕСТНОЙ схемы**
          Ответ:
          {{
            "decision": "general_query", # или, возможно, лучше классифицировать как что-то другое, если есть внешний классификатор
            "entity_name": ""
          ...
          Пример 9:
          Запрос: "Этапы "Схема операционной процедуры для операции "Откачка сыворотки"" 
          Ответ:
          {{
            "decision": "general_query", # или, возможно, лучше классифицировать как что-то другое, если есть внешний классификатор
            "entity_name": "Схема операционной процедуры для операции "Откачка сыворотки""
          }}
          
          Пример 10:
          Запрос: "Этапы Схема операционной процедуры для операции "Отделение сыворотки"" 
          Ответ:
          {{
            "decision": "general_query", # или, возможно, лучше классифицировать как что-то другое, если есть внешний классификатор
            "entity_name": "Схема операционной процедуры для операции "Отделение сыворотки""
          }}
        ...
        if decision == "scheme_steps_needed":
            self.logger.info(f"LLM решил вызвать агента для поиска ключевых sc-элементов. Сущность: '{entity_name}'")
            ...
            find_template_action, find_template_result = execute_agent(
                ...
                [
                    CommonIdentifiers.ACTION,
                    SearchModuleIdentifiers.ACTION_FIND_STAGES_LIST,
                ]
            )
            ...
            find_template_action_result = get_action_result(find_template_action)
            find_template_action_result_struct = ScStructure(set_node=find_template_action_result)
            result_string_link = next(iter(find_template_action_result_struct))
            result_string = get_link_content_data(result_string_link)

            # Окончен вызов агента

            # result_string = self.call_agent_get_string_result(entity_name, action_node, SearchModuleIdentifiers.ACTION_FIND_STAGES_LIST)

            # print("РЕЗУЛЬТАТЫ ПОИСКА: " + result_string)

            answer_prompt = f"""
                    Запрос пользователя: "{link_query}"
                    Полученная информация: "{result_string}"

                    Сначала идет номер этапа. Затем идет название этапа. И потом номер этапа, на который идет переход

                    Ты система интеллектуальной поддержки сотрудников. Сформируй на основе полученной ифнормации ответ для пользователя. Используй только полученную информацию.
                    """
            ...
            print(f"""Предметная область: Схемы и Процессы.
                  ОШИБКА: Никакой агент не был вызван.
              """)
            return False
